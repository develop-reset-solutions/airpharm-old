...
//$usr_id=$_REQUEST['usr_id'];
		...
		function Footer()
		{
			...
			//global $nombre_evaluador_ant;
			//global $apellidos_evaluador;
			global $fecha_cierre;
			//global $fecha_cierre_ant;
			...
			//if($cabecera=="competencias"){
				$this->Cell(90,4,'Evaluador: '.$nombre_evaluador.' '.$cabecera,'T',0,'L');
				$this->Cell(90,4,'Fecha cierre: '.$fecha_cierre.'','T',0,'L');
			//}
			/*
			if($cabecera=="observaciones"){
				$this->Cell(90,4,'Evaluador: '.$nombre_evaluador_ant.' '.$cabecera,'T',0,'L');
				$this->Cell(90,4,'Fecha cierre: '.$fecha_cierre_ant.'','T',0,'L');
			}*/
	  ...
	  if ($_SESSION['usr_perfil']=='Director RRHH'){
		...
		while($row_usr_dic=mysql_fetch_array($result_usr_dic)){
			...
			$fecha=date('d/m/Y');
			//$cambio="no";
			//if ($nombre_evaluador!=$row_vdic["usr_nombre"]." ".$row_vdic["usr_apellidos"]){
				$nombre_evaluador_ant = $nombre_evaluador;
				$fecha_cierre_ant = $fecha_cierre;
				//$cambio="si";
			//}
			/*	
			if ($apellidos_evaluador!=$row_vdic["usr_apellidos"]){
				$apellidos_evaluador_ant = $apellidos_evaluador;
			}
			$apellidos_evaluador_ant = $apellidos_evaluador;
			if ($fecha_cierre!=cambiaf_a_normal($row_vdic["du_fecha_cierre"])){
				$fecha_cierre_ant = $fecha_cierre;
			}
			*/
			$nombre_evaluador = $row_vdic["usr_nombre"]." ".$row_vdic["usr_apellidos"];
...
//Para los que tu eres su responsable 
$query_usr1="SELECT * FROM usuarios WHERE usr_baja=0 ORDER BY usr_apellidos, usr_nombre ASC";
$result_usr1=mysql_query($query_usr1) or die ("No se puede ejecutar la sentencia: ".$query_usr1);
while($row_usr1=mysql_fetch_array($result_usr1)){
