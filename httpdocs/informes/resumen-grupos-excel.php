...
$usr_id=$_POST['usr_id'];
$usuarios_sel=array();
unset($usuarios_sel); // porque ?
if($usr_id=='all'){
	$query="SELECT * FROM vdpo_cabeceras WHERE dpo_ano=".$_SESSION['ano']." ORDER BY usr_apellidos ASC, usr_nombre ASC";
	...
	$query="SELECT * FROM vdpo_cabeceras WHERE usr_cen_id=".$cen_id." AND dpo_ano=".$_SESSION['ano']." ORDER BY usr_apellidos ASC, usr_nombre ASC";
	$result=mysql_query($query) or die ("No se puede ejecutar la sentencia: ".$query);
}elseif(substr($usr_id,0,3)=='dep'){
	$dep_id=substr($usr_id,4,3);
	$query="SELECT * FROM vdpo_cabeceras WHERE usr_dep_id=".$dep_id." AND dpo_ano=".$_SESSION['ano']." ORDER BY usr_apellidos ASC, usr_nombre ASC";
	$result=mysql_query($query) or die ("No se puede ejecutar la sentencia: ".$query);
}else{
	$query="SELECT * FROM vdpo_cabeceras WHERE dpo_usr_id=".$usr_id." AND dpo_ano=".$_SESSION['ano'];
	$result=mysql_query($query) or die ("No se puede ejecutar la sentencia: ".$query);
}
$n_sel=0;

while($row=mysql_fetch_array($result)){
	if(($_SESSION['usr_perfil']=='Director General' or $_SESSION['usr_perfil']=='Administrador' or $_SESSION['usr_perfil']=='Director RRHH') or $usr_id<>'all' or ($usr_id=='all' and superior($_SESSION['usr_id'],$row['dpo_usr_id']))){
		$usuarios_sel[$n_sel]=$row['dpo_usr_id'];
		$n_sel++;
	}
	// var_dump($row);
	// die;
...
$objPHPExcel->getDefaultStyle()->getFont()
    	...
    	->setSize(12);
$num_usr=0;
foreach($usuarios_sel as $valueSelected){
	$num_usr++;
}
...
$usuario=array();
$resumen=array();
// porque los quitas si despues los has de usar?
// wtf // estaria bien poner explicacion que hace esto?
unset ($usuario); 
unset ($resumen);

$columnas=array('I','J','K','L','M','N','O','P','Q','R','S','T','U','V','W','X','Y','Z','AA','AB','AC','AD','AE','AF','AG','AH','AI','AJ','AK','AL','AM','AN','AO','AP','AQ','AR','AS','AT','AU','AV','AW','AX','AY','AZ','BA','BB','BC','BD','BE','BF','BG','BH','BI','BJ','BK','BL','BM','BN','BO','BP','BQ','BR','BS','BT','BU','BV','BW','BX','BY','BZ','CA','CB','CC','CD','CE','CF','CG','CH','CI','CJ','CK','CL','CM','CN','CO','CP','CQ','CR','CS','CT','CU','CV','CW','CX','CY','CZ','DA','DB','DC','DD','DE','DF','DG','DH','DI','DJ','DK','DL','DM','DN','DO','DP','DQ','DR','DS','DT','DU','DV','DW','DX','DY','DZ','EA','EB','EC','ED','EE','EF','EG','EH','EI','EJ','EK','EL','EM','EN','EO','EP','EQ','ER','ES','ET','EU','EV','EW','EX','EY','EZ');
...
foreach($usuarios_sel as $valueSelected){
	...
	$columna = columnFromIndex($n + 9);

	//var_dump($row);
	//die;
	//$usuario[$row['usr_id']] = $row;

	$objPHPExcel->setActiveSheetIndex(0)
    		    ->setCellValue($columna.'1',utf8_encode($row['usr_apellidos'].", ".$row['usr_nombre']));
	
	$objPHPExcel->getActiveSheet()->getStyle($columna.'1')->applyFromArray($celdanegrita);
	$objPHPExcel->getActiveSheet()->getStyle($columna.'1')
				->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
	$n++;

	// echo $columnas[$n] . ' -> ' . $columna;
	// echo '<hr>';
}
...
while($row_oa=mysql_fetch_array($result_oa)){
	foreach($usuarios_sel as $valueSelected){
		...
		if($num_dl){
			...
			// 1-21-2022
			// el $usuario no existe! ( se ha puesto continue para probar si funciona sin esto )
			// continue;
	...
	if($tiene_usr){
		...
		if($primero){
			...
			foreach($usuarios_sel as $valueSelected){
				...
				//if($valueSelected == 10091 || $valueSelected == 129 || $valueSelected == 90){
					//continue;
				//}
				...
				$n++;
				//echo("Valor: ".$valueSelected." ---- Nombre ".$row2['usr_nombre']." ---- Linea: ".$n."<br>");
			}
			$fila++;
