 ...
 function cambiar_competencia(){
	...
	var cambio=document.getElementById("cambio").value;
	 ...
	 if (cambio=="true"){
		...
		if (r == true) {
			window.location="show.php?com_id="+competencia+"&usr_id="+usr_id;
...
function cambiar_agrupado(){
	var competencia=document.getElementById("competencia").value;
	var usr_id = document.getElementById("user").value;
	window.location="show_agrupado.php?com_id="+competencia+"&usr_id="+usr_id;
	}
...
function cambiar_usuario(){
	...
	if (cambio=="true"){
		var r = confirm("Seguro que quiere salir sin guardar")
		if (r == true) {
			window.location="show.php?usr_id="+usr_id;
       ...
       <?php
        ...
        <tr>
          ...
          <td class="texto_10">
            ...
            <td style="background-color:#999999;"><?php 
				  ...
				  if ($com_id_ant){
					  $col++;
				  	?>
                    <a href="show.php?com_id=<?php echo $com_id_ant;?>&amp;usr_id=<?php echo $usr_id;?>"><img src="../../img/atras.png" width="25" height="25" ></a>
                     <?php }
				  if ($com_id_sig){
					  $col++;
				  	?>
                    
                   <a href="show.php?com_id=<?php echo $com_id_sig;?>&amp;usr_id=<?php echo $usr_id;?>"><img src="../../img/adelante.png" width="25" height="25"></a>
                    
                   <?php }
				  ?></td>
          
          <?php if ($dic_agrupado=="si"){?>
          ...
          <td class="texto_14" colspan=" <?php echo 3-$col;?>" style="width:200px;"><?php echo  $row_vdic['dic_nombre'];?></td>
          	...
          	<td class="texto_10"><input type="button" value="Cambiar a agrupado" onclick="cambiar_agrupado()" /></td>
		  <?php } else { ?>
	          <!--<td colspan=" <?php echo 4-$col;?>" style="background-color:#999999; width:400px;"></td>-->
              <td class="texto_14" colspan=" <?php echo 4-$col;?>" style="width:400px;"><?php echo  $row_vdic['dic_nombre'];?></td>
          <?php }?>
          ...
          <td class="texto_10"> Colaborador:
            ...
            <?php
			 if ($_SESSION['usr_perfil']=='Director RRHH'){
				...
				while($row_usr_dic=mysql_fetch_array($result_usr_dic)){
						...
						//calculo el número de competencias del diccionario 
						...
						$num_grados_ok=0;
						$query_grados_ok="SELECT duc_grado FROM vcom_dic_usr_com WHERE du_usr_id=".$row_usr_dic['du_usr_id']." AND du_dic_id=".$dic_id;
						//$query_grados_ok="SELECT duc_grado FROM com_dic_usr_com WHERE duc_usr_id=".$row_usr_dic['du_usr_id']." AND duc_dic_id=".$dic_id;
						...
						//si todos los grados de la competencias estan informadas el nombre sale en verde.
						?>
				...
				$query_grados="SELECT cd_com_id FROM com_com_dic WHERE cd_dic_id=".$dic_id;
				$result_grados=mysql_query($query_grados) or die ("No se puede ejecutar la sentencia: ".$query_grados);
				$num_grados = mysql_num_rows($result_grados);
				...
				$query_grados_ok="SELECT duc_grado FROM vcom_dic_usr_com WHERE du_usr_id=".$_SESSION['usr_id']." AND du_dic_id=".$dic_id;
				$result_grados_ok=mysql_query($query_grados_ok) or die ("No se puede ejecutar la sentencia: ".$query_grados_ok);
				while($row_grados_ok=mysql_fetch_array($result_grados_ok)){								
					if($row_grados_ok['duc_grado']!=""){$num_grados_ok++;}
				}
			?>
              ...
              <?php /* 						
					$query_usr_dic="SELECT du_usr_id FROM com_diccionarios_usuarios WHERE du_responsable=".$_SESSION['usr_id'];
					$result_usr_dic=mysql_query($query_usr_dic) or die ("No se puede ejecutar la sentencia: ".$query_usr_dic);
					while($row_usr_dic=mysql_fetch_array($result_usr_dic)){
						...
						$row_usr=mysql_fetch_array($result_usr)
					?>
              <option value="<?php echo $row_usr_dic['du_usr_id'];?>" <?php if ($row_usr_dic['du_usr_id']==$row_usr1['usr_id']){echo "selected";}?> >
			  <?php echo utf8_encode($row_usr['usr_apellidos'].", ".$row_usr['usr_nombre']);?></option>
              <?php }*/
			  //$query_usr1="SELECT * FROM usuarios WHERE usr_baja=0 and usr_id<>". $_SESSION['usr_id']." ORDER BY usr_apellidos, usr_nombre ASC";
  ...
  <form action="modify.php" method="post" enctype="multipart/form-data">
	...
	<?php
      ...
      <tr class="filas_subtotal">
        <td>
            <textarea id="observaciones" name="observaciones" class="observaciones" disabled><?php 
        ...
        <td>
            <select id="grado" name="grado" class="puntuacion" disabled>
        ...
        <td>
			...
			if($row['du_usr_id']!=""){
				...
				//$query_com_ant="SELECT * FROM vcom_dic_usr_com WHERE duc_com_id=". $com_id." AND du_usr_id=".$row['du_usr_id']." AND duc_com_id=".$row['duc_com_id']." AND dic_ano=".$ano_ant;
				...
				while($row_comp_act_com_dic=mysql_fetch_array($result_comp_act_com_dic)){
					...
					?>	
						<tr class="filas_subtotal">
							...
							<?php 
							if($dic_agrupado=="no"){
								echo utf8_encode($row_comp_act_com_dic['cacd_numero']);
							} else{
								echo $num;
							}
							?>. <?php echo utf8_encode($row_comp['comp_nombre']);?></td>
							<td class="celdas_subtotal">
                            	<?php 
                              ...
                              <select name="comp_<?php echo $j;?>_<?php echo $i;?>" id="comp_<?php echo $j;?>_<?php echo $i;?>" disabled onChange="cambio_no_agrupado()">
                                <option value="0"> </option>
