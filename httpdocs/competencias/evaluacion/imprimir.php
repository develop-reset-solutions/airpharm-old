...
function Footer()
{
	...
	$this->Cell(120,4,'Evaluador: '.$nombre_evaluador.' '.$apellidos_evaluador,'T',0,'L');
	
	$this->Cell(0,4,'Fecha cierre: '.$fecha_cierre.'','T',1,'R');
	*/
...
/*
	...
	$pdf->Cell(174,6,utf8_encode($row_vdic['dic_nombre']),'LRT',0,'C',true);
	$pdf->SetFont('Arial','B',8);
	$pdf->Cell(25,6,'ACTITUDES','LRT',0,'C',true);
	$pdf->Cell(40,6,'COMPORTAMIENTOS',1,0,'C',true);
	$pdf->Cell(30,6,'G. COMPETENCIAL',1,0,'C',true);
	$pdf->Cell(0,6,'',0,1,'C',true);
	
	$pdf->SetFont('Arial','B',8);
	$pdf->SetFillColor(254);
	$pdf->SetTextColor(0);
	$pdf->Cell(174,4,'','LRB',0,'L',true);
	$pdf->Cell(25,4,'','LRB',0,'L',true);
	$pdf->Cell(10,4,'1',1,0,'C',true);
	$pdf->Cell(10,4,'2',1,0,'C',true);
	$pdf->Cell(10,4,'3',1,0,'C',true);
	$pdf->Cell(10,4,'4',1,0,'C',true);
	$pdf->Cell(10,4,'A',1,0,'C',true);
	$pdf->Cell(10,4,'B',1,0,'C',true);
	$pdf->Cell(10,4,'C',1,0,'C',true);
	...
	*/
		...
		if ( $com_antiguo!=$row_lin['cd_orden']){
			...
			if ($act_antiguo!=$row_lin['acd_grado']){
				...
				while($row_comp_act_com_dic=mysql_fetch_array($result_comp_act_com_dic)){
					/*
					//$i++;
					$query_comp="SELECT * FROM com_comportamientos WHERE comp_id=".$row_comp_act_com_dic['cacd_comp_id']." ORDER BY cacd_numero";
					...
					while($row_comp=mysql_fetch_array($result_comp)){
						$query_comp="SELECT * FROM com_dic_usr_comp WHERE ducp_du_id=".$du_id." AND ducp_comp_id=".$row_lin['cacd_comp_id'];
						$result_comp=mysql_query($query_comp) or die ("No se puede ejecutar la sentencia: ".$query_comp);
			...
			/*
				...
				if ($row_lin['acd_grado']=="B"){
					...
					/*$query_com="SELECT * FROM com_dic_usr_com WHERE duc_du_id=".$row_vdic['du_id']." AND duc_com_id=".$row_lin['cd_com_id'];
	...
	/*
	$query_obs="SELECT * FROM com_dic_usr_com WHERE duc_usr_id=".$usr_id." AND duc_dic_id=".$row_vdic['dic_id']." AND duc_com_id=".$row_lin['cd_com_id'];
	$result_obs=mysql_query($query_obs) or die ("No se puede ejecutar la sentencia: ".$query_obs);
	while($row_obs=mysql_fetch_array($result_obs)){}*/
	//$pdf->Cell(269,6,$num_obs,'LRT',1,'C',true);
	...
	//$pdf->Cell(269,4,'','LRB',1,'L',true);
	} else {
	//para dic_agrupado="si" //////////////////////////////////////////////////////////////////////////////////////////////////////////////
