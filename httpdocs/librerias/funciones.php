...
function por_obtencion($dl_id,$trimestre){
	...
	*/
	...
	// Funcion Carlos
	// Horquilla positiva
	if($row['oa_horquilla_min'] < $row['oa_horquilla_max']){
		if($valor_medicion >= $row['oa_horquilla_max']){
			$multiplicador = $valor_medicion;
		}elseif($valor_medicion <= $row['oa_horquilla_min']){
			$multiplicador = 0;
		}elseif($valor_medicion < $row['oa_horquilla_max'] && $valor_medicion > $row['oa_horquilla_min']){
			$multiplicador = ($row['oa_horquilla_max']*($valor_medicion - $row['oa_horquilla_min'])/($row['oa_horquilla_max'] - $row['oa_horquilla_min']));
		}
	// Horquilla negativa
	}elseif($row['oa_horquilla_min'] > $row['oa_horquilla_max']){
		if($valor_medicion >= $row['oa_horquilla_min']){
			$multiplicador = 0;
		}elseif($valor_medicion <= $row['oa_horquilla_max']){
			$multiplicador = 100;
		}elseif($valor_medicion > $row['oa_horquilla_max'] && $valor_medicion < $row['oa_horquilla_min']){
			$multiplicador = (($row['oa_horquilla_min'] - $valor_medicion)*100)/($row['oa_horquilla_min'] - $row['oa_horquilla_max']);
		}
	// Horquilla igual
	}elseif($row['oa_horquilla_max'] == $row['oa_horquilla_min']){
		if($valor_medicion >= $row['oa_horquilla_max']){
			$multiplicador = 100;
		}else{
			$multiplicador = 0;
	...
	$total=($multiplicador*$row['dl_peso'])/100;
	return $total;
}
