...
$usr_id=$_POST['usr_id'];
if($usr_id=='all'){
	$query="SELECT * FROM vdpo_cabeceras WHERE dpo_ano=".$_SESSION['ano']." AND usr_baja='0' ORDER BY usr_apellidos ASC, usr_nombre ASC";
	...
	$query="SELECT * FROM vdpo_cabeceras WHERE usr_dep_id=".$dep_id." AND dpo_ano=".$_SESSION['ano']." AND usr_baja='0' ORDER BY usr_apellidos ASC, usr_nombre ASC";
	$result=mysql_query($query) or die ("No se puede ejecutar la sentencia: ".$query);
}else{
	$query="SELECT * FROM vdpo_cabeceras WHERE dpo_usr_id=".$usr_id." AND usr_baja='0' AND dpo_ano=".$_SESSION['ano'];
	$result=mysql_query($query) or die ("No se puede ejecutar la sentencia: ".$query);
}
...
if($num_lin){
	...
	while($row_lin=mysql_fetch_array($result_lin)){
		...
		$pdf->SetFillColor(180,253,180);
		$pdf->y=$yorigen;
		$pdf->Cell(0,$altura,imprimir_valor((por_obtencion($row_lin['dl_id'])*$valor_sim)/100).'€',1,1,'R',true);
		$pdf->SetFillColor(231,229,227);
 }
...
/*	$pdf->SetFillColor(180,253,180);
	$pdf->SetTextColor(0);
*/	$pdf->Cell(10,4,number_format($total_sim,2,',','.'),0,1,'R',true);
	$pdf->AddPage('L');
	$pdf->SetFont('Arial','B',8);
	$pdf->SetTextColor(255);
	$pdf->SetFillColor(0);
