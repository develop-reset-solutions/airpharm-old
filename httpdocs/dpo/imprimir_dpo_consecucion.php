...
function Header(){
  	...
  	$this->Cell(0,4,'Importe de simulaci�n: '.number_format($valor_sim,0,',','.').' �','B',0,'R');
		$this->Ln();
  	$this->SetFont('Arial','B',10);
}
function Footer(){
  ...
  $this->Cell(0,4,'P�gina: '.$this->PageNo().'/{nb}','T',0,'R');
}
...
$pdf->AddPage('L');
$n_lin=1;
$query_lin="SELECT * FROM vdpo_lineas WHERE dl_dpo_id=".$dpo_id." AND obj_tipo LIKE 'Objetivo de compa�ia' ORDER BY dl_peso DESC, obj_descripcion ASC";
$result_lin=mysql_query($query_lin) or die ("No se puede ejecutar la sentencia: ".$query_lin);
...
if($num_lin){
	...
	$pdf->SetFillColor(0);
	$pdf->Cell(197,4,'A nivel de compa�ia',0,0,'L',true);
	$pdf->Cell(10,4,$peso['Objetivo de Compa��a'].' %',0,0,'R',true);
...
$query_lin="SELECT * FROM vdpo_lineas WHERE dl_dpo_id=".$row['dpo_id']." AND (obj_tipo LIKE 'Para el Comit� de Direcci�n' OR obj_tipo LIKE '".utf8_decode('Mandos Intermedios')."') ORDER BY dl_peso DESC, obj_descripcion ASC";
		$result_lin=mysql_query($query_lin) or die ("No se puede ejecutar la sentencia: ".$query_lin);
		$num_lin=mysql_num_rows($result_lin);
		if($num_lin){
	$pdf->SetFont('Arial','B',8);
	$pdf->SetTextColor(255);
	$pdf->SetFillColor(0);
		if($row['usr_categoria']=='Direcci�n'){
          $tipo="A nivel de comit� de direcci�n";
           }else{
          $tipo="A nivel de Mandos Intermedios";
          }
	$pdf->Cell(197,4,$tipo,0,0,'L',true);
	$pdf->Cell(10,4,$peso['Para el Comit� de Direcci�n']+$peso['Mandos Intermedios'].' %',0,0,'R',true);
	$pdf->Cell(0,4,'',0,1,'R',true);
	$pdf->SetFillColor(120);
	$pdf->Cell(4,4,'',0,0,'C',true);
	$pdf->Cell(70,4,'Objetivo',0,0,'L',true);
